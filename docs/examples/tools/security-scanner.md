...
```json
{
  "vulnerabilities": {
    ...
    "medium": [
      ...
      "jsonwebtoken@8.5.0 - Verification bypass"
    ]
  }
}
```
...
```html
<!-- Found 8 instances -->
<div>Hello ${username}</div>  <!-- Need HTML escape -->
...
```html
<!-- Found 2 instances -->
<a href="/user/${userId}">Profile</a>  <!-- Need URL encode -->

Fix: <a href="/user/<%= encodeURIComponent(userId) %>">
```
