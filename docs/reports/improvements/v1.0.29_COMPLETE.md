...
- ✅ Safe patterns: `AKIA[REDACTED]`, `github_token_[EXAMPLE]`, `stripe_key_[EXAMPLE]`
...
```typescript
// Before (blocked):
'AKIA[EXAMPLE_PATTERN]'; // Looked like real AWS key
'github_token_[EXAMPLE]'; // Looked like real GitHub token

// After (allowed):
'AKIA' + 'X'.repeat(16); // Obviously fake: AKIAXXXXXXXXXXXXXXXX
'github_' + 'X'.repeat(36); // Obviously fake: all X's
'stripe_' + 'Z'.repeat(24); // Obviously fake: all Z's
```
