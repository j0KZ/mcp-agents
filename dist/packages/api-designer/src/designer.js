 ...
 * ```
 */
export function generateOpenAPI(config, endpoints) {
    try {
        const spec = {
            openapi: '3.0.3',
            info: {
 ...
 * ```
 */
export function designRESTEndpoints(resources, config) {
    try {
 ...
 * ```
 */
export function createGraphQLSchema(config, customTypes) {
    try {
        const schema = {
            types: customTypes || [],
            queries: [],
 ...
 * @private
 */
function convertSchemaToSDL(schema) {
    const lines = [];
    // Add types
 ...
 * ```
 */
export function generateAPIClient(spec, options) {
    try {
        let code = '';
 ...
 * @private
 */
function generateTypeScriptRestClient(spec, options) {
    const lines = [];
    // Add imports
 ...
 * @private
 */
function mapSchemaToTSType(schema) {
    if (schema.$ref) {
 ...
 * ```
 */
export function generateMockServer(spec, config) {
    try {
 ...
 * @private
 */
function generateExpressMockServer(spec, config) {
    const lines = [];
