...
function outer() {
  ...
  describe('renameVariable', () => {
    ...
    it('should handle exception during variable renaming', () => {
      const result = renameVariable({ code: null as any, oldName: 'test', newName: 'newTest' });
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
    });
  });
  ...
  describe('applyDesignPattern', () => {
    ...
    it('should reject invalid pattern', () => {
      ...
      const result = applyDesignPattern({ code, pattern: 'invalid' as any });
      expect(result.success).toBe(false);
      expect(result.error).toBeDefined();
    });
