...
export function searchTools(options = {}) {
    ...
    for (const tool of TOOL_REGISTRY) {
        ...
        if (category && tool.category !== category)
            continue;
        if (frequency && tool.frequency !== frequency)
            continue;
        if (server && tool.server !== server)
            continue;
        if (loadedOnly && !tool.loaded)
            continue;
        let relevance = 0;
        let matchedKeywords = [];
        if (query) {
...
export function suggestTools(context) {
    ...
    if (contextLower.includes('refactor') ||
    ...
    if (contextLower.includes('documentation') ||
