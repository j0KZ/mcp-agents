 ...
 */
export function validateNoTraversal(inputPath) {
    // Check for path traversal patterns BEFORE normalization
    // This catches attempts that would be resolved away
    if (inputPath.includes('..')) {
        throw new PathValidationError('Path traversal detected: path contains ".." sequence', inputPath);
    }
    // Normalize the path to resolve any . segments (but not ..)
    const normalized = path.normalize(inputPath);
    // Double-check after normalization
    if (normalized.includes('..')) {
        throw new PathValidationError('Path traversal detected: path contains ".." sequence after normalization', inputPath);
    }
 ...
 */
export function validatePath(inputPath, allowedRoot) {
 ...
 */
export function validateFilePath(filePath, allowedRoot) {
 ...
 */
export function validateDirectoryPath(dirPath, allowedRoot) {
