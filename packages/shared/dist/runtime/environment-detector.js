...
export class EnvironmentDetector {
    ...
    static detectIDE() {
        ...
        if (process.env.CURSOR_VERSION)
            return 'cursor';
        if (process.env.WINDSURF_VERSION)
            return 'windsurf';
        if (process.env.QODER_VERSION)
            return 'qoder';
        if (process.env.ROO_CODE_VERSION)
            return 'roo-code';
        if (process.env.VSCODE_PID)
            return 'vscode';
        if (process.env.MCP_IDE)
            return process.env.MCP_IDE; // Explicit override
        ...
        try {
            ...
            if (parentProcess.includes('cursor'))
                return 'cursor';
            if (parentProcess.includes('code'))
                return 'vscode';
            if (parentProcess.includes('qoder'))
                return 'qoder';
            if (parentProcess.includes('windsurf'))
                return 'windsurf';
        }
        catch {
     ...
     */
    static isFilesystemCaseSensitive() {
