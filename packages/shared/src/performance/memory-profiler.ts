...
export class MemoryProfiler extends EventEmitter {
   ...
   */
  startProfiling(label = 'default'): void {
  ...
  endProfiling(): {
    ...
    const checkpointDeltas = new Map<string, MemoryDelta>();
    for (const [label, snapshot] of this.checkpoints.entries()) {
      checkpointDeltas.set(label, {
        ...
        percentChange:
          ((snapshot.heapUsed - this.baseline.heapUsed) / this.baseline.heapUsed) * 100,
        label,
   ...
   */
  detectMemoryLeak(label = 'default'): MemoryLeakReport {
   ...
   */
  private calculateLinearRegression(snapshots: MemorySnapshot[]): {
    slope: number;
    r2: number;
  } {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        if (report.leakReport.suspected) {
          console.warn(
            `[Memory Leak Warning] ${profileLabel}: ${report.leakReport.recommendation}`
          );
        }
