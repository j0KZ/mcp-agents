...
export class PredictiveAnalyzer extends EventEmitter {
  ...
  private async detectPatterns(history: HistoryEntry[]): Promise<PatternDetectionResult> {
    ...
    if (perfIssues.length > 0) {
      ...
      const recentIssues = perfIssues.filter(h =>
        Date.now() - h.timestamp.getTime() < 30 * 24 * 60 * 60 * 1000 // Last 30 days
      ).length;

      const oldIssues = perfIssues.filter(h =>
        Date.now() - h.timestamp.getTime() >= 30 * 24 * 60 * 60 * 1000
      ).length;
    ...
    for (const file of codebase.files) {
      if (file.changeFrequency > 5) { // Changed >5 times per week
        ...
        const probability = Math.min(
          baseProbability * (1 + file.changeFrequency / 10),
          0.95
        );
  ...
  private async predictVulnerabilities(
    ...
    const authFiles = codebase.files.filter(f =>
  ...
  async validatePredictions(
    ...
    for (const bugPred of latestPrediction.likelyBugs) {
      const actual = actualOutcomes.find(o =>
        o.type === 'bug' && o.location === bugPred.file
      );
