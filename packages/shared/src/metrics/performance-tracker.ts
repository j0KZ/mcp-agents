...
export class PerformanceTracker extends EventEmitter {
  ...
  constructor(dataPath: string = './metrics-data') {
    super();
   ...
   */
  async track(metric: ToolMetric): Promise<void> {
  ...
  async recordFeedback(
    ...
    const metric = this.metrics.find(
      m => m.toolId === toolId && m.timestamp.toISOString() === operationId
    );
   ...
   */
  getToolMetrics(toolId: string, period?: { start: Date; end: Date }): ToolMetric[] {
  ...
  private detectPatterns(): Array<{
    ...
    const failurePatterns = new Map<string, string[]>();
   ...
   */
  private updateAggregations(metric: ToolMetric): void {
   ...
   */
  private async learnFromFeedback(metric: ToolMetric): Promise<void> {
    if (!metric.humanFeedback) return;
