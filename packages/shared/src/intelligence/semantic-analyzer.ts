...
export class SemanticAnalyzer {
   ...
   */
  private async detectPurpose(ast: any, context?: any): Promise<string> {
    ...
    traverse(ast, {
      ...
      ClassDeclaration: (path: any) => {
        const name = path.node.id?.name || '';
        if (name.includes('Controller')) purposes.push('Controller');
    ...
    if (purposes.length === 0) {
      ...
      if (context?.fileName) {
        if (context.fileName.includes('controller')) return 'Controller';
        if (context.fileName.includes('service')) return 'Service layer';
        if (context.fileName.includes('util')) return 'Utility function';
        if (context.fileName.includes('test')) return 'Test suite';
      }
   ...
   */
  private detectSideEffects(ast: any): SideEffect[] {
   ...
   */
  private extractDependencies(ast: any): Dependency[] {
   ...
   */
  private analyzeComplexity(ast: any): ComplexityAnalysis {
    let cognitive = 0;
    let cyclomatic = 1;
    let maxDepth = 0;
    let currentDepth = 0;
    ...
    traverse(ast, {
      enter(path: any) {
        ...
        if (t.isIfStatement(path.node)) cognitive += 1 + (path.node.alternate ? 1 : 0);
        if (t.isForStatement(path.node)) cognitive += 2;
        if (t.isWhileStatement(path.node)) cognitive += 2;
        if (t.isSwitchStatement(path.node)) cognitive += 2;
        if (t.isTryStatement(path.node)) cognitive += 2;
        if (t.isConditionalExpression(path.node)) cognitive += 1;

        // Cyclomatic complexity
        if (t.isIfStatement(path.node)) cyclomatic++;
        if (t.isForStatement(path.node)) cyclomatic++;
        if (t.isWhileStatement(path.node)) cyclomatic++;
        if (t.isSwitchCase(path.node)) cyclomatic++;
        if (t.isCatchClause(path.node)) cyclomatic++;
        if (t.isConditionalExpression(path.node)) cyclomatic++;
        if (t.isLogicalExpression(path.node) && path.node.operator === '&&') cyclomatic++;
        if (t.isLogicalExpression(path.node) && path.node.operator === '||') cyclomatic++;
   ...
   */
  private detectPatterns(ast: any, code: string): string[] {
   ...
   */
  private detectAntiPatterns(ast: any, code: string): string[] {
  ...
  private generateSuggestions(analysis: any): string[] {
    ...
    if (analysis.sideEffects.filter((e: SideEffect) => e.type === 'async').length > 3) {
      suggestions.push('Consider using Promise.all for parallel async operations');
    }

    // Security suggestions
    const criticalEffects = analysis.sideEffects.filter((e: SideEffect) => e.risk === 'high');
  ...
  private categorizeCode(purpose: string, patterns: string[]): CodeIntent['category'] {
    if (purpose.includes('Auth') || patterns.includes('Authentication')) return 'security';
    if (purpose.includes('Database') || purpose.includes('Repository')) return 'data';
    if (purpose.includes('Controller') || purpose.includes('API')) return 'business';
    if (purpose.includes('Service') || purpose.includes('Helper')) return 'utility';
  ...
  private detectSensitivity(name: string): DataFlow['sensitivity'] {
    ...
    if (sensitive.some(s => lower.includes(s))) return 'sensitive';
    if (lower.includes('email') || lower.includes('phone')) return 'private';
    return 'public';
  }

  private findValidations(node: any, paramName: string): string[] {
  ...
  private extractActions(ast: any): string[] {
  ...
  private calculateConfidence(factors: any): number {
    ...
    if (factors.hasComments) confidence += 0.1;
    if (factors.hasTests) confidence += 0.1;
    if (factors.patternMatches > 0) confidence += Math.min(0.1, factors.patternMatches * 0.02);
    if (factors.knownPurpose) confidence += 0.1;
  ...
  private extractAction(node: any): string {
  ...
  private extractUrl(node: any): string {
  ...
  private classifyDependency(source: string): 'internal' | 'external' | 'system' {
    if (source.startsWith('.')) return 'internal';
    if (source.startsWith('node:') || ['fs', 'path', 'crypto'].includes(source)) return 'system';
    return 'external';
  }
  ...
  private detectObserver(ast: any, code: string): boolean {
    return this.patterns.get('observer')!.test(code);
  }

  private detectRepository(ast: any): boolean {
    let found = false;
    traverse(ast, {
      ClassDeclaration: (path: any) => {
  ...
  private detectDependencyInjection(ast: any): boolean {
    let found = false;
    traverse(ast, {
      ClassDeclaration: (path: any) => {
  ...
  private detectBuilder(ast: any, code: string): boolean {
    return code.includes('.with') && code.includes('.build()');
  }

  private detectMiddleware(ast: any, code: string): boolean {
    return code.includes('next()') || code.includes('middleware');
  }

  private detectGodObject(ast: any): boolean {
    let methodCount = 0;
    traverse(ast, {
      ClassDeclaration: (path: any) => {
  ...
  private detectCallbackHell(ast: any): boolean {
    let maxDepth = 0;
    let currentDepth = 0;
    traverse(ast, {
      CallExpression: {
  ...
  private detectMagicNumbers(ast: any): boolean {
    let magicCount = 0;
    traverse(ast, {
  ...
  private detectDuplication(code: string): boolean {
    ...
    for (let i = 0; i < lines.length - 5; i++) {
      ...
      if (duplicates.has(chunk)) return true;
      duplicates.add(chunk);
    }
    return false;
  }

  private detectLongParameterList(ast: any): boolean {
    let hasLong = false;
    traverse(ast, {
      FunctionDeclaration: (path: any) => {
  ...
  private detectDeepNesting(ast: any): boolean {
    let maxDepth = 0;
    let currentDepth = 0;
    traverse(ast, {
      BlockStatement: {
  ...
  private detectUnusedVariables(ast: any): boolean {
  ...
  private describeOperation(node: any): string {
  ...
  private getTransformationChain(path: any): string[] {
