...
/**
 * Internal stats structure for tracking
 */
interface ToolStats {
  calls: number;
  totalTime: number;
...
export class ToolUsageTracker {
  private usage: Map<string, ToolStats> = new Map();
  private sessionStart: Date;
  private toolFrequencies: Map<string, ToolFrequency> = new Map();
  private toolServers: Map<string, MCPServerName> = new Map();
  ...
  getFrequencyReport(registeredTools?: string[]): FrequencyReport {
    ...
    const mediumFrequency = metrics.filter(
      m => m.callCount >= MEDIUM_THRESHOLD && m.callCount < HIGH_THRESHOLD
    );
   ...
   */
  private generateRecommendations(metrics: ToolUsageMetrics[]): FrequencyRecommendation[] {
    const recommendations: FrequencyRecommendation[] = [];

    for (const metric of metrics) {
      const currentFreq = this.toolFrequencies.get(metric.toolName);
      if (!currentFreq) continue;

      let suggestedFreq: ToolFrequency | undefined;
      let reason = '';
  ...
  getServerUsage(): ServerUsageSummary[] {
    const serverStats = new Map<
      MCPServerName,
      { calls: number; time: number; errors: number; tools: Map<string, number> }
    >();
 ...
 */
export function createTrackerWithRegistry(
  toolRegistry: Array<{ name: string; frequency: ToolFrequency; server: MCPServerName }>
): ToolUsageTracker {
  const tracker = new ToolUsageTracker();
