...
const TOOLS: Tool[] = [
  {
    ...
    description:
      'Generate OpenAPI 3.0 specification from API configuration. Creates comprehensive API documentation with endpoints, schemas, security, and more.',
    inputSchema: {
      type: 'object',
      properties: {
        config: {
  ...
  },
  {
    ...
    description:
      'Design RESTful API endpoints following best practices. Automatically generates CRUD operations for resources with proper HTTP methods, status codes, and pagination.',
    inputSchema: {
      type: 'object',
      properties: {
        resources: {
  ...
  },
  {
    ...
    description:
      'Create GraphQL schema with types, queries, mutations, and subscriptions. Generates complete SDL with proper type definitions and resolvers.',
    inputSchema: {
      type: 'object',
      properties: {
        config: {
  ...
  },
  {
    ...
    description:
      'Generate API client code in various languages (TypeScript, Python, etc.). Supports multiple HTTP libraries and includes type definitions.',
    inputSchema: {
      type: 'object',
      properties: {
        spec: {
  ...
  },
  {
    name: 'validate_api',
    description:
      'Validate API design against best practices and standards. Checks for security, versioning, documentation, and common pitfalls.',
    inputSchema: {
      type: 'object',
      properties: {
        spec: {
  ...
  },
  {
    ...
    description:
      'Generate mock server code for testing and development. Creates a runnable server with mock responses based on API specification.',
    inputSchema: {
      type: 'object',
      properties: {
        spec: {
  ...
  try {
    switch (name) {
      case 'generate_openapi': {
        const { config, endpoints } = args as {
      ...
      case 'design_rest_api': {
        const { resources, config } = args as {
          resources: string[];
          config: APIDesignConfig;
        };
      ...
      case 'create_graphql_schema': {
        const { config, customTypes } = args as {
          config: APIDesignConfig;
