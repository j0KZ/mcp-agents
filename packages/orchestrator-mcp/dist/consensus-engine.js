...
export class ConsensusEngine extends EventEmitter {
    ...
    strategies = new Map();
    consensusHistory = [];
    trustScores = new Map(); // Dynamic trust scoring
    constructor() {
        super();
     ...
     */
    unanimousConsensus(request, participants) {
     ...
     */
    weightedMajorityConsensus(request, participants) {
     ...
     */
    calculateWeight(participant, opinion, request) {
        let weight = 1.0;
     ...
     */
    expertLedConsensus(request, participants) {
        if (!request.domain) {
            return this.weightedMajorityConsensus(request, participants);
        }
     ...
     */
    evidenceBasedConsensus(request, participants) {
        ...
        const dissenting = scoredOpinions
            .slice(1)
     ...
     * Calculate agreement based on evidence similarity
     */
    calculateEvidenceAgreement(scoredOpinions) {
        if (scoredOpinions.length <= 1)
            return 1.0;
     ...
     */
    byzantineConsensus(request, participants) {
     ...
     */
    delphiMethodConsensus(request, participants) {
        ...
        while (rounds < maxRounds) {
            ...
            const interim = this.weightedMajorityConsensus({ ...request, opinions: currentOpinions }, participants);
        ...
        // Final round
        const finalResult = this.weightedMajorityConsensus({ ...request, opinions: currentOpinions }, participants);
        return {
            ...finalResult,
            method: 'delphi-method',
            explanation: [
     ...
     */
    refineOpinions(opinions, interim) {
     ...
     */
    hybridConsensus(request, participants) {
        const results = [];
        // Try multiple methods
        const methods = ['weighted-majority', 'expert-led', 'evidence-based'];
        ...
        return {
            ...
            evidence: this.aggregateEvidence(request.opinions.filter(o => JSON.stringify(o.value) === bestValue))
        };
    }
    /**
     * Calculate average confidence
     */
    calculateAverageConfidence(opinions) {
     ...
     */
    calculateWeightedConfidence(opinions, participants) {
        let weightedSum = 0;
        let totalWeight = 0;
        for (const opinion of opinions) {
            const participant = participants.get(opinion.participant);
     ...
     */
    compareEvidenceStrength(a, b) {
        const strengthOrder = { 'weak': 0, 'moderate': 1, 'strong': 2 };
        return (strengthOrder[a] || 0) -
            (strengthOrder[b] || 0);
    }
    /**
     * Find dissenting opinions
     */
    findDissenting(opinions, finalValue) {
        return opinions
     ...
     */
    async updateTrustScores(result) {
    ...
    getConsensusMetrics() {
        ...
        const participantPerformance = new Map();
