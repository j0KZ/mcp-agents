...
export interface Specialization {
  ...
  experience: number; // Total tasks handled
  successRate: number; // 0-1
  recentPerformance: number[]; // Last 10 performance scores
  skills: Skill[];
...
export interface TrainingPath {
  ...
  requirements: Requirement[];
  estimatedTime: number; // Hours of experience needed
}

export interface ToolProfile {
  toolId: string;
  specializations: Map<string, Specialization>;
  primaryFocus: string; // Main specialization
  secondaryFocus: string[]; // Additional areas
...
export class SpecializationSystem extends EventEmitter {
  ...
  private domainExperts: Map<string, string[]> = new Map(); // domain -> expert tools
  ...
  private readonly SPECIALIZATION_THRESHOLD = 75; // Score needed to specialize
  private readonly EXPERT_THRESHOLD = 90; // Score needed for expert status
   ...
   */
  private addSkillNode(id: string, node: SkillNode): void {
   ...
   * Create tool profile
   */
  private createToolProfile(toolId: string, config: any): void {
    const profile: ToolProfile = {
      toolId,
      specializations: new Map(),
      primaryFocus: config.primaryFocus,
      secondaryFocus: config.specializations.map((s: any) => s.name).slice(1),
      learningStyle: {
   ...
   */
  private createSpecialization(name: string, domain: string, level: number): Specialization {
    return {
      id: `${name}-${Date.now()}`,
      name,
      domain,
   ...
   */
  private getSkillsForSpecialization(name: string, level: number): Skill[] {
    const skills: Skill[] = [];
   ...
   */
  private createTrainingPath(specialization: string, currentLevel: number): TrainingPath {
   ...
   */
  private getNextLevel(current: number): number {
    if (current < 50) return 50;
    if (current < 70) return 70;
   ...
   * Add domain expert
   */
  private addDomainExpert(domain: string, toolId: string): void {
   ...
   */
  async assignTask(requirements: TaskRequirements): Promise<TaskAssignment> {
   ...
   */
  private findCandidates(requirements: TaskRequirements): ToolCandidate[] {
   ...
   */
  private calculateMatchScore(profile: ToolProfile, requirements: TaskRequirements): number {
    let score = 0;
   ...
   */
  private getComplexityScore(profile: ToolProfile, complexity: string): number {
   ...
   */
  private getAverageSpecializationLevel(profile: ToolProfile): number {
    if (profile.specializations.size === 0) return 0;
   ...
   */
  private calculateRecentPerformance(profile: ToolProfile): number {
   ...
   */
  private estimateSuccess(profile: ToolProfile, requirements: TaskRequirements): number {
    const matchScore = this.calculateMatchScore(profile, requirements);
   ...
   */
  private getSpecializationBonus(profile: ToolProfile, domain: string): number {
    let maxBonus = 0;
  ...
  private scoreCandidateForSelection(candidate: ToolCandidate, _requirements: TaskRequirements): number {
    ...
    return (
      candidate.matchScore * matchWeight +
   ...
   */
  private calculateLevelChange(success: boolean, score: number, currentLevel: number): number {
    if (!success) {
   ...
   */
  private async updateSkills(profile: ToolProfile, task: any): Promise<void> {
    for (const [_, spec] of profile.specializations.entries()) {
      if (spec.domain === task.domain) {
  ...
  private async awardCertification(profile: ToolProfile, program: CertificationProgram): Promise<void> {
    const certification: Certification = {
      ...
      requirements: program.requirements.map(r => ({
        type: r.type as 'tasks' | 'success-rate' | 'skill-level' | 'peer-review',
        target: r.target,
   ...
   */
  private shouldChangeSpecialization(profile: ToolProfile): boolean {
   ...
   */
  private async suggestSpecializationChange(profile: ToolProfile): Promise<void> {
    // Find highest level specialization
    let bestSpec: Specialization | null = null;
    let bestLevel = 0;
   ...
   */
  private updateDomainExperts(): void {
    // Clear and rebuild
  ...
  generateReport(): {
    ...
    topPerformers: { toolId: string; score: number }[];
  } {
    const domainCoverage = new Map<string, number>();
    const certificationStats = new Map<string, number>();
    const performers: { toolId: string; score: number }[] = [];
