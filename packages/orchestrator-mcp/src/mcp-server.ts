...
class OrchestratorServer {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
      tools: [
        ...
        },
        {
          name: 'run_sequence',
          description:
            'Execute a custom sequence of MCP tools with full control over steps and dependencies',
          inputSchema: {
            type: 'object',
            properties: {
              steps: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        if (name === 'run_sequence') {
          return await this.runSequence(args.steps as WorkflowStep[]);
        }
   ...
   */
  private async runSequence(steps: WorkflowStep[]) {
