...
class OrchestratorServer {
  ...
  private loadedTools: Set<string> = new Set(); // Track loaded deferred tools
      ...
      try {
        ...
        if (name === 'run_workflow') {
          return await this.runWorkflow(
            ...
            (args.response_format as ResponseFormat) || 'detailed'
          );
        }

        if (name === 'run_sequence') {
          return await this.runSequence(
        ...
        if (name === 'list_workflows') {
          return this.listWorkflows((args.response_format as ResponseFormat) || 'detailed');
        }

        // Phase 3: Deferred Loading meta-tools
        if (name === 'search_tools') {
          return this.searchToolsHandler(
            args as ToolSearchOptions & { response_format?: ResponseFormat }
          );
        }

        if (name === 'load_tool') {
          return this.loadToolHandler(
            args.toolName as string,
            args.server as MCPServerName | undefined,
            (args.response_format as ResponseFormat) || 'detailed'
          );
        }
        ...
        if (name === 'list_capabilities') {
          return this.listCapabilitiesHandler(
   ...
   */
  private async runSequence(steps: WorkflowStep[], response_format: ResponseFormat = 'detailed') {
    ...
    const formatted = formatResponse(
      ...
      { format: response_format },
      {
        ...
        concise: r => ({
          ...
          duration: r.duration,
          steps: truncateArray(
            r.steps.map(s => ({ name: s.name, success: s.success, duration: s.duration })),
            'concise'
          ),
   ...
   */
  private listWorkflows(response_format: ResponseFormat = 'detailed') {
    ...
    const formatted = formatResponse(
      ...
      { format: response_format },
      {
        minimal: r => ({
          ...
          ids: r.workflows.map((w: any) => w.id),
        }),
        concise: r => ({
          workflows: truncateArray(
            r.workflows.map((w: any) => ({ id: w.id, name: w.name, description: w.description })),
            'concise'
          ),
        }),
        detailed: r => r,
   ...
   */
  private searchToolsHandler(options: ToolSearchOptions & { response_format?: ResponseFormat }) {
   ...
   */
  private listCapabilitiesHandler(server?: string, response_format: ResponseFormat = 'detailed') {
      ...
      const formatted = formatResponse(
        ...
        { format: response_format },
        {
          ...
          concise: r => ({
            server: r.server,
            tools: truncateArray(
              r.tools.map((t: { name: string; frequency: string }) => ({
                name: t.name,
    ...
    const formatted = formatResponse(
      ...
      { format: response_format },
      {
        ...
        concise: r => ({
          categories: truncateArray(
            r.categories.map((c: { name: string; toolCount: number }) => ({
              name: c.name,
