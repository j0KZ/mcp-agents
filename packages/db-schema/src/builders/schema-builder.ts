...
export function buildMongoSchema(
  entities: string[],
  relationships: Relationship[],
  options: SchemaDesignOptions
): DatabaseSchema {
...
export function extractEntities(requirements: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (multiMatch) {
      ...
      if (!entities.includes(entity1)) entities.push(entity1);
      if (!entities.includes(entity2)) entities.push(entity2);
    }
  }
...
export function extractRelationships(requirements: string, _entities: string[]): Relationship[] {
