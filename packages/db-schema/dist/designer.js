...
function extractRelationships(requirements, entities) {
    ...
    const relationshipPatterns = [
        { pattern: /(\w+)\s+has\s+many\s+(\w+)/gi, type: 'ONE_TO_MANY' },
        { pattern: /(\w+)\s+belongs\s+to\s+(\w+)/gi, type: 'ONE_TO_ONE' },
    ];
...
function buildSQLSchema(entities, relationships, options) {
    const tables = [];
...
function buildMongoSchema(entities, relationships, options) {
    const collections = [];
    for (const entity of entities) {
        const fields = [];
...
function generateSQLUpMigration(schema) {
...
function generateSQLDownMigration(schema) {
...
function generateMongoUpMigration(schema) {
...
function generateMongoDownMigration(schema) {
...
function generateMermaidDiagram(schema, options) {
...
function generateSQLRecords(table, count) {
    const records = [];
...
function generateMongoRecords(collection, count) {
    const records = [];
    for (let i = 0; i < count; i++) {
        const record = {};
...
function generateMockValue(name, type, index) {
...
function validateSQLSchema(schema, errors, warnings) {
    for (const table of schema.tables || []) {
...
function estimateNormalForm(schema) {
    const tables = schema.tables || [];
    // Simple heuristic
    const hasArrays = tables.some(t => t.columns.some(c => c.type === 'ARRAY'));
    if (hasArrays)
        return '1NF'; // Array columns violate 1NF
