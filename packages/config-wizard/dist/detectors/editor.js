...
export async function detectInstalledEditors() {
    const editors = [];
    if (await detectClaudeCode())
        editors.push('claude-code');
    if (await detectCursor())
        editors.push('cursor');
    if (await detectWindsurf())
        editors.push('windsurf');
    if (await detectVSCode())
        editors.push('vscode');
    if (await detectRooCode())
        editors.push('roo');
    if (await detectQoder())
        editors.push('qoder');
    return editors;
}
...
export function getEditorConfigPath(editor) {
    if (!editor)
        return null;
