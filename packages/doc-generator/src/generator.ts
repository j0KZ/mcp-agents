...
export async function generateApiDocs(
  ...
  try {
    ...
    if (config.includeTOC !== false) {
      sections.push('## Table of Contents');
      sections.push('');
      if (allClasses.length > 0) sections.push('- [Classes](#classes)');
      if (allInterfaces.length > 0) sections.push('- [Interfaces](#interfaces)');
      if (allFunctions.length > 0) sections.push('- [Functions](#functions)');
      sections.push('');
    }
...
export async function generateChangelog(
  ...
  try {
    ...
    const commits: ChangelogCommit[] = gitOutput
      .trim()
      .split('\n')
      .filter(line => line)
      .map(line => {
    ...
    const groupedCommits = new Map<string, ChangelogCommit[]>();
