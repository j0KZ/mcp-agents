 ...
 */
import { SemanticAnalyzer, ExecutionSandbox, DomainKnowledgeBase, ExplanationEngine, PerformanceTracker, MessageBus } from '@j0kz/shared';
export class IntelligentTestGeneratorV2 {
     ...
     */
    async generateIntelligentTests(code, context = {}) {
    ...
    generateTestCases(intent) {
        ...
        testCases.push({
            ...
            expectedBehavior: {
                shouldThrow: false,
                sideEffects: intent.sideEffects.map((s) => s.type)
            }
        });
        ...
        for (const input of intent.inputs) {
            if (input.type === 'number') {
                ...
                testCases.push({ inputs: [''] }, { inputs: [' '.repeat(1000)] }, { inputs: ['special!@#$%^&*()'] });
            }
            else if (input.type === 'array') {
                testCases.push({ inputs: [[]] }, { inputs: [[null, undefined]] }, { inputs: [new Array(10000).fill(0)] });
            }
        }
        // Error cases
        testCases.push({ inputs: [null], expectedBehavior: { shouldThrow: true } }, { inputs: [undefined], expectedBehavior: { shouldThrow: true } });
        return testCases;
    }
    /**
     * Generate happy path inputs based on intent
     */
    generateHappyPathInputs(intent) {
        return intent.inputs.map((input) => {
            switch (input.type) {
     ...
     */
    async generateTestsFromInsights(insights) {
        ...
        if (intent.sideEffects.some((s) => s.type === 'database')) {
     ...
     */
    calculateCoverage(intent, executionResults) {
     ...
     */
    calculateQuality(tests, intent, executionResults) {
    ...
    generateExplanation(data) {
        ...
        return this.explanationEngine.createUnifiedExplanation(new Map([
            ...
            ['Framework Insights', this.explanationEngine.explainFrameworkInsights(frameworkInsights.framework || 'general', frameworkInsights)],
            ['Execution Results', this.explanationEngine.explainExecutionResults(executionResults, tests.code, [])],
            ['Test Strategy', this.explanationEngine.explainTestGeneration([], tests.coverage, {
                    edgeCases: quality.edgeCaseCoverage > 70,
                    mocking: quality.mockingStrategy > 70,
                    asyncTesting: intent.sideEffects.some((s) => s.type === 'async')
                })]
        ]));
    }
    /**
     ...
     */
    async learnFromFeedback(tests, feedback) {
...
//# sourceMappingURL=intelligent-generator-v2.js.map