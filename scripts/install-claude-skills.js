...
function showBanner() {
  console.log('\n' + '='.repeat(50));
  console.log('    🧠 Claude Universal Skills Installer');
...
function ensureClaudeFolder() {
  ...
  if (!fs.existsSync(claudeDir)) {
    console.log('📁 Creating .claude folder for Claude configuration...');
...
async function installSkills() {
  ...
  console.log(`📥 Installing ${SKILLS_CONFIG.skills.length} universal skills...\n`);
  ...
  for (let i = 0; i < SKILLS_CONFIG.skills.length; i++) {
    ...
    try {
      ...
      console.log(` ✅ (${skill.time}, ${skill.impact} impact)`);
      successful++;
    } catch (error) {
      console.log(` ❌ Failed`);
      failed++;
...
⚠️ This skill could not be downloaded automatically.
...
async function main() {
  ...
  try {
    ...
    console.log('\n📊 Summary:');
    console.log(`  • Skills installed: ${result.successful}/${SKILLS_CONFIG.skills.length}`);
    console.log(`  • Location: ${path.relative(process.cwd(), result.location)}/`);
    console.log(`  • Size: ~500KB (optimized markdown)`);
    ...
    console.log('\n📖 Documentation:');
    console.log('  • Skills index: .claude/universal-skills/INDEX.md');
    console.log('  • Usage guide: .claude/universal-skills/USAGE.md');
    console.log('  • Online docs: https://github.com/j0KZ/mcp-agents/wiki');
